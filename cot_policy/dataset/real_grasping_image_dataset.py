...
class GraspingImageDataset(BaseImageDataset):
    def __init__(
        self,
        shape_meta: dict,
        dataset_path: str,
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        for key in self.rgb_keys:
            ...
            del data[key]
        for key in self.lowdim_keys:
            # Keep x and y only for the planar task
        ...
        torch_data = {
            ...
            "action": torch.from_numpy(action),  # Keep x and y only for the planar task
        }
        return torch_data
...
def zarr_resize_index_last_dim(zarr_arr, idxs):
    ...
    # Slice the array according to the provided indices
    actions = actions[..., idxs]

    # Create a new Zarr array with the resized shape
    new_shape = zarr_arr.shape[:-1] + (len(idxs),)
    new_zarr_arr = zarr.empty(new_shape, dtype=zarr_arr.dtype)

    # Copy the sliced data into the new array
    new_zarr_arr[:] = actions

    return new_zarr_arr


def _get_replay_buffer(dataset_path, shape_meta):
    # parse shape meta
    rgb_keys = list()
    lowdim_keys = list()
    ...
    for key, attr in obs_shape_meta.items():
        ...
        if type == "rgb":
            ...
            if "pose" in key:
                assert tuple(shape) in [(2,), (6,), (8,)]

    action_shape = tuple(shape_meta["action"]["shape"])
    assert action_shape in [(2,), (6,), (7,), (12,)]
