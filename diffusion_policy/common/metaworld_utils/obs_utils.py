...
import diffusion_policy.common.metaworld_utils.tensor_utils as TU
...
def sample_random_image_crops(
    ...
    crop_inds_h = (
        max_sample_h * torch.rand((*source_im.shape[:-3], num_crops), device=device)
    ).long()
    crop_inds_w = (
        max_sample_w * torch.rand((*source_im.shape[:-3], num_crops), device=device)
    ).long()
    crop_inds = torch.cat(
