...
import diffusion_policy.common.metaworld_utils.tensor_utils as TensorUtils
...
class SequenceDataset(torch.utils.data.Dataset):
    ...
    def load_demo_info(
        self, filter_by_attribute=None, demos=None, n_demos=None, episode_mask=None
    ):
        """
        Args:
        ...
        if n_demos is not None:
            assert n_demos <= len(
                self.demos
            ), "asking for more demos than available in the dataset"
            masked_demos = []
            for demo in range(n_demos):
                if episode_mask[demo]:
                    masked_demos.append(self.demos[demo])
        self.demos = masked_demos
        # sort demo keys
    ...
    def get_data(self, mode="limits", **kwargs):
        ...
        # Initialize the dictionary to hold concatenated observations and actions
        concatenated_traj = {
            k: [] for k in self.obs_keys
        }  # For obs_keys like 'corner_rgb', 'robot_states'
        concatenated_traj["actions"] = []  # For actions

        # Process the first demonstration
        ep = self.demos[0]
        obs_traj = {
            k: self.hdf5_file["data/{}/obs/{}".format(ep, k)][()].astype("float32")
            for k in self.obs_keys
        ...
        obs_traj = ObsUtils.process_obs_dict(obs_traj)
        action_traj = self.hdf5_file["data/{}/actions".format(ep)][()].astype("float32")

        # Append the first demo's observations and actions
        for k in self.obs_keys:
            concatenated_traj[k].append(obs_traj[k])
        concatenated_traj["actions"].append(action_traj)

        print("SequenceDataset: normalizing observations...")

        # Process the remaining demonstrations
        for ep in tqdm(self.demos[1:]):
            obs_traj = {
                k: self.hdf5_file["data/{}/obs/{}".format(ep, k)][()].astype("float32")
                for k in self.obs_keys
            ...
            obs_traj = ObsUtils.process_obs_dict(obs_traj)
            action_traj = self.hdf5_file["data/{}/actions".format(ep)][()].astype(
                "float32"
            )

            # Append each demo's observations and actions
            for k in self.obs_keys:
                concatenated_traj[k].append(obs_traj[k])
            concatenated_traj["actions"].append(action_traj)

        # Convert lists to numpy arrays for final concatenation
        for k in self.obs_keys:
            concatenated_traj[k] = np.concatenate(concatenated_traj[k], axis=0)

        # Concatenate actions into a single array
        concatenated_traj["actions"] = np.concatenate(
            concatenated_traj["actions"], axis=0
        )

        # Now concatenated_traj is a dictionary with all observations and actions
        # {
        #     'corner_rgb': array of shape [total_demo_length, 3, 224, 224],
        #     'robot_states': array of shape [total_demo_length, 8],
        #     'actions': array of shape [total_demo_length, action_dim]  # adjust action_dim accordingly
        # }
        # data = {
        #     "actions": concatenated_traj["actions"],
        #     "robot_states": concatenated_traj["robot_states"],
        # }
    ...
    def get_item(self, index):
        ...
        if self.lowdim_obs_seq_length is None:
            ...
            high_dim_keys = [
                ...
                if not ObsUtils.key_is_obs_modality(key, "low_dim")
            ]
            low_dim_keys = [
                key
                for key in self.obs_keys
            ...
            meta["obs"].update(
                self.get_obs_sequence_from_demo(
                    demo_id,
                    index_in_demo=index_in_demo,
                    keys=low_dim_keys,
